...
class CustomPaginationActionsTable extends React.Component {
  ...
  getData = () => {
    Axios.get(urlApi + '/cart', {params : {idUser : this.props.id}})
    .then((res) => {
  ...
  getItem = () => {
    var arr = []
    for(var i = 0 ; i < this.state.rows.length ; i++){
      var data = {namaProduk : this.state.rows[i].namaProduk , harga : this.state.rows[i].harga, qty : this.state.rows[i].qty, discount : this.state.rows[i].discount}
      arr.push(data)
    }
    return arr
  }
            ...
            <Paper className={classes.root}>
                ...
                <Table className={classes.table}>
                    ...
                    {rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row,index) => (
                        ...
                        <TableCell>{formatMoney(row.qty*(row.harga - (row.harga*(row.discount/100))))}</TableCell>
